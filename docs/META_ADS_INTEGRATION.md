...
| `META_PROVISIONING_LOGIN_CONFIG_ID` | Optional dedicated Meta Login configuration ID for the one-time recent-active-account assignment. It must allow `ads_read`, `ads_management`, and `business_management`; the application still grants only `ANALYZE` to selected accounts. When unset, the inventory configuration is used. |
| `META_PROVISIONING_REDIRECT_URI` | HTTPS same-origin `/meta-active-account-connection` URL registered in the dedicated user-token Login configuration. Its fragment is cleared immediately and posted only to the protected same-origin completion route. |
...
`/meta-active-account-connection` is a separate, explicit one-time assignment route. It requests `ads_read`, `ads_management`, and `business_management` for the operator's current Meta session because Meta requires `ads_management` to change an ad account's assigned-user list. The application repeats the recent-spend check and assigns only accounts with `Insights.spend > 0` to the two pre-created Employee pools. It uses the `ANALYZE` task only: it contains no campaign, budget, creative, or account-creation path. The personal token and raw account IDs remain in server memory; the result cookie contains aggregate counts only. The route verifies the configured Business Portfolio and exact pool names, resolves the app-scoped pool IDs server-side without persisting or rendering them, then posts an assignment. It fails closed above 500 candidates and never changes the existing `Nasmedia API User 2026` or `Conversions API System User`.
